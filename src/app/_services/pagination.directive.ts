...
export class PaginatorDirective implements DoCheck, AfterViewInit {
	...
	private readonly showTotalPages: number;
	...
	private buildPageNumbers = () => {
		...
		for (let index = startIndex; index < totalPages - 1; index = index + 1) {
			if (
				...
				||
				(this.rangeStart  && index >= this.rangeStart && this.rangeEnd && index <= this.rangeEnd)
				||
				...
				if (this.rangeEnd && index > this.rangeEnd && !dots[0]) {
					this.renderer.insertBefore(
						actionContainer,
				...
				if (this.rangeEnd && index < this.rangeEnd && !dots[1]) {
					this.renderer.insertBefore(
						actionContainer,
