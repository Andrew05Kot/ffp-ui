...
export class CompressCropImageService {
  ...
  static calcRatio(image): number {
    ...
    const sideOfPhotoForComparing = heightIsBigger ? CompressCropImageService.defaultHeight
      : CompressCropImageService.defaultWidth;
  ...
  async resizeImg(file: File, readerResult: ArrayBuffer | null): Promise<string> {
    ...
    const ratio = CompressCropImageService.calcRatio(readerResult);
